...
#define FONT_S "-*-times-*-i-*-140-100-*"
#define FONT_M "-*-times-*-i-*-180-100-*"
#define FONT_L "-*-times-*-i-*-240-100-*"
...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        while(XCheckTypedEvent(disp, Expose, &event) || resize) {
            if(resize) {
                if(win_h < 70)
                    font = XLoadQueryFont(disp, FONT_S);
                else if(win_h < 120)
                    font = XLoadQueryFont(disp, FONT_M);
                else
                    font = XLoadQueryFont(disp, FONT_L);
...
void drawbar(void) {    /* This is called on every frame */
...
void redraw(void) {     /* This is called only only when needed */
